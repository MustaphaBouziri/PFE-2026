...
// 6. Health check — no auth, no forwarding
app.get('/health', (req, res) => {
  res.json({ status: 'ok', uptime: process.uptime(), timestamp: new Date().toISOString() });
});

// 7. Preflight handler — express/cors handles OPTIONS automatically,
//    but this ensures 200 with no body for all preflight routes
app.options('/api/{*path}', cors(corsOptions));

// 8. THE MAIN ROUTE — forward everything to BC
app.all('/api/{*path}', forwardRequest);

// 9. 404 for anything that isn't /api/* or /health
app.use((req, res) => {
  res.status(404).json({ error: 'Not found', message: `Route ${req.method} ${req.originalUrl} does not exist on this proxy.` });
});

// 10. Global error handler (catches synchronous throws)
