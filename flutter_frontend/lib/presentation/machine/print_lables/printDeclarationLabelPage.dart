...
class DeclarationLabelPage extends StatefulWidget {
  final OperationStatusAndProgressModel operationData;
  final int quantity;

  const DeclarationLabelPage({
    super.key,
  ...
  @override
  State<DeclarationLabelPage> createState() => _DeclarationLabelPageState();
}

class _DeclarationLabelPageState extends State<DeclarationLabelPage> {
  ...
  Future<void> _print() async {
    final format = _isLandscape ? PdfPageFormat.a4.landscape : PdfPageFormat.a4.portrait;
    final svg = Barcode.dataMatrix().toSvg(_barcodeData, width: 120, height: 120);

    await Printing.layoutPdf(
      onLayout: (_) async {
        final doc = pw.Document();

        // one page per declared unit
            ...
            build: (_) => pw.Row(
              ...
              children: [
                ...
                pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text('Declaration Label', style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)),
                    pw.SizedBox(height: 8),
                    _pdfRow('Order',   widget.operationData.prodOrderNo),
                    _pdfRow('Machine', widget.operationData.machineNo),
                    _pdfRow('Item No', widget.operationData.itemNo),
                    _pdfRow('Item',    widget.operationData.itemDescription),
                    _pdfRow('Label',   '${i + 1} / ${widget.quantity}'),
                  ],
                ),
              ],
            ),
  ...
  Widget _infoRow(String label, String value) => Padding(
    ...
    child: RichText(text: TextSpan(
      ...
      children: [
        ...
        TextSpan(text: value, style: const TextStyle(color: Color(0xFF64748B))),
      ],
    )),
  );
    ...
    child: Container(
      ...
      child: _isLandscape
          ? Row(
              ...
              children: [
                BarcodeWidget(barcode: Barcode.dataMatrix(), data: _barcodeData, width: 100, height: 100),
          ...
          : Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Declaration Label', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                const SizedBox(height: 16),
                BarcodeWidget(barcode: Barcode.dataMatrix(), data: _barcodeData, width: 120, height: 120),
                const SizedBox(height: 16),
  ...
  Widget _info({required bool showTitle}) => Column(
    ...
    children: [
      if (showTitle) ...[
        const Text('Declaration Label', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
      ],
      _infoRow('Order',   widget.operationData.prodOrderNo),
      _infoRow('Machine', widget.operationData.machineNo),
      _infoRow('Item No', widget.operationData.itemNo),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Declaration Labels', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        actions: [
          Row(children: [
            const Text('Landscape', style: TextStyle(fontSize: 13)),
            Switch(value: _isLandscape, onChanged: (val) => setState(() => _isLandscape = val)),
          ]),
              ...
              child: Column(
                children: [
                  Container(
                    ...
                    child: Text(
                      '${widget.quantity} copies will be created',
